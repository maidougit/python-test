...
logging.basicConfig(level=logging.DEBUG,#控制台打印的日志级别
                    filename='F:\\python-test\\file\image\\log\\bohe-food-detail-006.log',
...
def batchDealHttpsByObj(row):
    # red = redis_connect()
    code = row['code']
    # if red.exists("food_detail:code:{0}".format(code)):
    #     logging.info("code:food_detail:{0}".format(code) + "已存在")
    # else:
    each = {}
    id = row['id']
    name = row['name']
    ...
    ingredient = urllib.request.urlopen(ingredientjson.format(code))
    each['ingredient_only'] = ingredient.read().decode('utf-8')
    # red.set("food_detail:code:{0}".format(code), each)
    print("update info", row )
    logging.info("update info " +  str(row))
    updateOneData(each)
...
def updateOneData(each):
    updateOneDb = db()
    updateSql = "update bohejiankang set food_detail=%s, ingredient_only=%s where id=%s"
    # dealdata = tuple(tuple([y for y in x]) for x in data)
    ...
    try:
        ...
        print(updateSql.format(each['id'], each['food_detail'], each['ingredient_only']))
        delrow = []
        delrow.append(each['food_detail'])
        delrow.append(each['ingredient_only'])
        delrow.append(each['id'])
        cursor.execute(updateSql, tuple(delrow))
        # 提交
        updateOneDb.commit()
    except Exception as e:
        print(e)
...
#分片处理
def slices(list):
    deal_slices_pool = ThreadPool(4)

    dealresults = deal_slices_pool.map(batchDealHttpsByObj, list)
...
# redis 链接
def redis_connect():
    pool = redis.ConnectionPool(host='192.168.1.119', password=123456,  port=6379, db=1)
...
if __name__ == '__main__':
    ...
    print(len(dealdb))
    #分片处理
    list = [dealdb[i:i + 20] for i in range(0, len(dealdb), 20)]
    # dealresults = pool.map(batchDealHttpsByObj, dealdb)
    dealresults = pool.map(slices, list)
    pool.close()
    pool.join()
