...
doctor ={"儿科":["儿科综合","儿内科","小儿呼吸科","儿外科","小儿妇科","小儿皮肤科","小儿精神科","儿童保健科","儿童五官科"],"妇产科":[],"口腔科":[],"皮肤性病科":[],"营养科":[],"全科":[]}
...
def request(department,ref,province,page,limit,city,subDepartment):
    ...
    with urllib.request.urlopen(req, context=context) as response:
        ...
        print('pages: ', pages)
        if pages == 1:
            requestData(department, ref, province, 1, limit, city, subDepartment)
        else:
...
def requestData(department, ref, province, page, limit, city, subDepartment):
    ...
    with urllib.request.urlopen(req, context=context) as response:
        ...
        print('data', data)
        dealData(data, province,city,department,subDepartment)

def dealData(data,  province,city,department,subDepartment):
    for each in data:
     print(each)
     row = {}
     ...
     row['subDepartment'] = subDepartment
     row['hospital'] = each['hospital']
     if each['metrics']:
         if len(each['metrics']) == 1:
             if each['metrics'][0]['key'] == '咨询量':
                 row['metricskey1'] = each['metrics'][0]['key']
                 row['metricsval1'] = each['metrics'][0]['value']
                 row['metricskey2'] = ''
                 row['metricsval2'] = ''
             else:
                 row['metricskey2'] = each['metrics'][0]['key']
                 row['metricsval2'] = each['metrics'][0]['value']
                 row['metricskey1'] = ''
                 row['metricsval1'] = ''
         elif len(each['metrics']) == 2:
             row['metricskey1'] = each['metrics'][0]['key']
             row['metricsval1'] = each['metrics'][0]['value']
             row['metricskey2'] = each['metrics'][1]['key']
             row['metricsval2'] = each['metrics'][1]['key']
     else:
         row['metricskey1'] = ''
         row['metricsval1'] = ''
         row['metricskey2'] = ''
     ...
     if each['illnessKeywords']:
         illnessKeywords = each['illnessKeywords'].strip()
         illnessKeywords = illnessKeywords.replace("\n", "")
         row['illnessKeywords'] = illnessKeywords
     else:
         row['illnessKeywords'] = ''
     if len(each['tags']):
...
if __name__ == '__main__':
    city_list = []
    city_path = 'D:\study\python-test\file\json\city.json'
    flag = os.path.isfile(city_path)
    with open(r'D:\study\python-test\file\json\city.json', 'r', encoding='gbk')as fp:
        json_data = json.load(fp)
        list = json_data['provinces']
        for each in list:
            provinces = each['name']
            cities = each['cities']
            for city in cities:
                row = {}
                row['province'] = provinces
                row['city'] = city['name']
                city_list.append(row)
    # D:\study\python-test\file\csv
    f = open(r'D:\study\python-test\file\csv\doctor-20201028.csv', 'w+', newline='', encoding='utf-8-sig')
    ...
    # doctor = {"营养科": []}
    #
    # city_list = [{"province":"上海","city":"上海市"}]
    list = []
    for k,v in doctor.items():
        if v:
           for item in v:
               for city_and_province in city_list:
                   data = {
                       "department": k,
                       "ref": '/wap/qie/doctor/find',
                       "province": city_and_province['province'],
                       "page": 1,
                       "limit": 200,
                       "city": city_and_province['city'],
                       "subDepartment": item
                   }
                   list.append(data)
        else:
